module sha256(
    ...
    // FIX: Use localparam (not reg + initial) so Quartus synthesizes K as a
    // compile-time constant, not a block RAM.  The reg+initial version was
    // for iverilog simulation compatibility only (see comment in original).
    // romstyle = "logic" forces a LUT-based combinational ROM, which gives
    // zero read latency and is required for the continuous 'assign' below.
    ...
    // K is now localparam so K[timer[5:0]] resolves to a constant mux,
    // not a RAM read — no latency, no initialization ambiguity.
